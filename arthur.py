...
def negamax(board_old, board_new, depth, alpha, beta, color):
    ...
    for move in board_new.get_moves():
        ...
        if board.active != board_new.active:
            val = -negamax(board_new, board, depth - 1, -beta, -alpha, -color)
        else:
            val = negamax(board_new, board, depth, alpha, beta, color)
...
def move_function(board, depth=7):
    def search(move):
        ...
        if board_new.active == board.active:
            return negamax(board, board_new, depth, -INFINITY, INFINITY, 1)
        else:
            return negamax(board, board_new, depth, -INFINITY, INFINITY, -1)
